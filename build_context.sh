...
{
  echo -e "Please Wait ..." 
  echo -n "Making virtual root fs compression ["
  tar -Jcf ${target_rootfs_file}   $1  --checkpoint=100 --checkpoint-action=dot 
  test $? -eq 0   || {
    echo -e "Fail to make compression "
  } && {
...
{  
   #TODO : To be remove  -> virtual_rootfs_extraction 
   local context_dir=$1 
   if [[ -d  ${context_dir}  ]] ; then 
     echo -ne "Importing to Docker Images Registry:\n\t|-> Will be  saved as <\x1b[1;33m${image_name}\x1b[0m>\n\t|"
     tar -C  ${context_dir}  -c . --checkpoint=.50 | docker import - ${image_name} 2>&1 /dev/null
     test  $? -eq 0  && {
